...
class CalibratedLiveScanner:
    ...
    def draw_ui_overlay(
        self, frame, face_idx, is_stable, stability_progress, message, avg_confidence=0.0
    ):
        ...
        stability_text = (
            f"Stability: {stability_progress:.0%} | Avg Confidence: {avg_confidence:.0%}"
        )
        cv2.putText(
            frame, stability_text, (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.5, stability_color, 1
        )
        ...
        cv2.putText(
            frame, calib_status, (w - 300, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1
        )
        ...
        for i, instruction in enumerate(instructions):
            y_pos = h - 80 + i * 25
            cv2.putText(
                frame, instruction, (20, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (200, 200, 200), 1
            )
    ...
    def scan_face(self, face_idx):
        ...
        while not face_captured:
                ...
                if is_stable:
                    ...
                    if auto_capture and stable_duration >= self.auto_capture_delay:
                        ...
                        if is_valid:
                            ...
                            face_captured = True
                            print(
                                f"✅ {self.face_labels[face_idx]} auto-captured! (Confidence: {avg_confidence:.0%})"
                            )
            ...
            self.draw_ui_overlay(
                frame, face_idx, is_stable, stability_progress, message, avg_confidence
            )
