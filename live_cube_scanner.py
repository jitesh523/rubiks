...
class LiveCubeScanner:
    ...
    def draw_live_grid(
        self, frame, start_x, start_y, grid_size, colors, is_stable, stability_progress
    ):
        ...
        for i in range(3):
            for j in range(3):
                if colors and i < len(colors) and j < len(colors[i]):
                    ...
                    cv2.putText(
                        frame, text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, text_color, 2
                    )
    ...
    def draw_ui_overlay(self, frame, face_idx, is_stable, stability_progress, message, colors=None):
        ...
        stability_color = (
            (0, 255, 0)
            if is_stable
            else (0, 255, 255) if stability_progress > 0.5 else (255, 255, 255)
        )
        stability_text = f"Stability: {stability_progress:.0%}"
        cv2.putText(
            frame, stability_text, (20, 105), cv2.FONT_HERSHEY_SIMPLEX, 0.6, stability_color, 2
        )
        ...
        cv2.rectangle(
            frame,
            (bar_x, bar_y),
            (bar_x + int(bar_width * stability_progress), bar_y + 15),
            stability_color,
            -1,
        )
        ...
        if message:
            cv2.putText(
                frame, message, (20, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2
            )
        ...
        for i, instruction in enumerate(instructions):
            y_pos = h - 80 + i * 25
            cv2.putText(
                frame, instruction, (20, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1
            )

        # Face progress indicator
        progress_text = f"Progress: {len(self.captured_faces)}/6 faces captured"
        cv2.putText(
            frame, progress_text, (w - 350, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2
        )
        ...
        while not face_captured:
            ...
            if has_unknowns:
                ...
                if auto_capture:
                    remaining = max(
                        0,
                        self.auto_capture_delay
                        - (time.time() - (self.last_stable_time or time.time())),
                    )
            ...
            self.draw_live_grid(
                frame, start_x, start_y, grid_size, stable_colors, is_stable, stability_progress
            )
            self.draw_ui_overlay(
                frame, face_idx, is_stable, stability_progress, message, stable_colors
            )
    ...
    def solve_and_display(self):
        ...
        if error:
            ...
            self.speak_async(
                f"Solution found with {len(solution)} moves. Check the terminal for step by step instructions."
            )
