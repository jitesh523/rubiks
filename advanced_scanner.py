...
class AdvancedCubeScanner:
    def __init__(self, use_voice=True):
        ...
        self.stability_threshold = 8  # Frames needed for stability
        ...
        for i in range(3):
            for j in range(3):
                ...
                if colors and i < len(colors) and j < len(colors[i]):
                    ...
                    overlay = frame.copy()
                    cv2.rectangle(overlay, (cell_x + 5, cell_y + 5), 
    ...
    def draw_ui_elements(self, frame, face_index, face_name, is_stable, stability_progress, message=""):
        ...
        # Stability indicator
        stability_color = (0, 255, 0) if is_stable else (0, 165, 255) if stability_progress > 0.5 else (255, 255, 255)
        stability_text = f"Stability: {stability_progress:.0%}"
        cv2.putText(frame, stability_text, (10, 75), cv2.FONT_HERSHEY_SIMPLEX, 0.5, stability_color, 1)
        ...
        cv2.rectangle(frame, (bar_x, bar_y), (bar_x + int(bar_width * stability_progress), bar_y + 10), stability_color, -1)
        
        # Status message
        if message:
            cv2.putText(frame, message, (10, 105), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        ...
        for i, instruction in enumerate(instructions):
            y_pos = h - 60 + i * 20
            cv2.putText(frame, instruction, (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200, 200, 200), 1)
    ...
    def scan_face_advanced(self):
        ...
        print(f"\\n📸 Scanning face {self.current_face_index + 1}/6: {face_name}")
        ...
        while not face_captured:
            ...
            if not has_unknowns:
                ...
                if len(self.recent_detections) >= self.stability_threshold:
                    ...
                    if is_stable and not self.is_stable:
                        ...
                        # Validate the face
                        is_valid, message = self.color_detector.validate_face_colors(stable_colors)
                        ...
                        self.stability_counter = min(self.stability_counter + 1, self.stability_threshold * 2)
                    else:
                        self.is_stable = False
            ...
            # Draw enhanced grid
            self.draw_enhanced_grid(frame, (roi_x, roi_y, roi_size, roi_size), 
                                  display_colors, self.is_stable, stability_progress)
            
            # Determine status message
            message = ""
            if has_unknowns:
                message = "⚠️ Improve lighting - unknown colors detected"
            elif self.is_stable:
            ...
            # Draw UI
            self.draw_ui_elements(frame, self.current_face_index, face_name, 
                                self.is_stable, stability_progress, message)
            ...
            if key == ord(' '):  # Manual capture
                if self.is_stable and self.stable_colors:
                    ...
                    print("⚠️ Face not stable enough for capture")
    ...
    def scan_all_faces_advanced(self):
        """Scan all 6 faces with advanced detection"""
        print("\\n🎲 === Advanced Rubik's Cube Scanner === 🎲")
        print("🚀 Features: Live preview, Auto-capture, Color validation")
        print("\\n💡 Controls:")
        print("  • SPACE: Manual capture")
        print("  • A: Toggle auto-capture")
        print("  • Q: Quit")
        print("\\n🎯 Face order: U(White) → R(Red) → F(Green) → D(Yellow) → L(Orange) → B(Blue)")
        
        self.speak_async("Advanced cube scanner ready. Show me the white face to begin.")
        
        for i in range(6):
            self.current_face_index = i
            face_data = self.scan_face_advanced()
            ...
            if i < 5:
                next_face = self.face_colors[i + 1]
                print(f"✅ Face {i + 1}/6 complete! Next: {next_face}")
                self.speak_async(f"Great! Now show me the {next_face} face")
                time.sleep(1)
        
        print("🎉 All faces scanned successfully!")
        self.speak_async("All faces scanned. Processing cube.")
        return self.cube_faces
    
    def convert_to_kociemba_format(self, cube_faces):
        """Convert detected colors to Kociemba solver format"""
        # Create color mapping based on center squares
        color_map = {}
        face_centers = {}
        ...
        for face_name in self.face_names:
            if face_name in cube_faces:
                center_color = cube_faces[face_name][1][1]  # Center of 3x3
                face_centers[face_name] = center_color
        
        # Create mapping from colors to face letters
        color_to_face = {}
        ...
        # Build cube string
        cube_string = ""
        for face_name in self.face_names:  # U, R, F, D, L, B order
            face_data = cube_faces[face_name]
            for row in face_data:
                for color in row:
                    if color in color_to_face:
                        cube_string += color_to_face[color]
                    else:
                        print(f"⚠️ Unknown color '{color}' - using U as default")
                        cube_string += 'U'
