...
from api.models.requests import CubeFacesRequest, CubeStateRequest
from api.models.responses import MoveExplanationResponse, SolutionResponse
...
async def solve_from_faces(request: CubeFacesRequest):
    ...
    """
    try:
        result = await solver_service.solve_cube_faces(request.faces, request.use_ml_detection)
...
async def explain_move(move: str):
    ...
    try:
        ...
        return MoveExplanationResponse(move=move, explanation=explanation, direction_hint=None)
    except Exception as e:
