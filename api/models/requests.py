...
class CubeStateRequest(BaseModel):
    ...
    cube_string: str = Field(
        ..., min_length=54, max_length=54, description="54-character cube state in URFDLB notation"
    )
...
class ColorPredictionRequest(BaseModel):
    ...
    model_config = {"json_schema_extra": {"examples": [{"rgb": [200, 30, 30]}]}}
...
class TrainModelRequest(BaseModel):
    ...
    confidence_threshold: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Confidence threshold for predictions"
    )
