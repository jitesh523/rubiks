...
class CubeScanner:
    ...
    def display_scanned_face(self, face_colors, face_name="Scanned Face"):
        ...
        cv2.putText(
            grid_img,
            "Press A to Accept, R to Retry, Any key to continue",
    ...
    def scan_face(self):
        ...
        while scanning:
            ...
            grid_color = (
                (0, 255, 0) if not captured else (0, 255, 255)
            )  # Green -> Cyan when captured
            ...
            cv2.putText(
                frame, progress_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2
            )
    ...
    def display_cube_net(self):
        ...
        for face_name, (start_x, start_y) in positions.items():
            if face_name in self.cube_faces:
                ...
                cv2.putText(
                    net_img,
                    f"{face_name} ({self.face_colors[self.face_names.index(face_name)]})",
                    label_pos,
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.5,
                    (255, 255, 255),
        ...
        cv2.putText(
            net_img,
            "Complete Cube Net - Press any key to continue",
            (10, 25),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.8,
            (255, 255, 255),
