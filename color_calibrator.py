    ...
    def run_calibration_mode(self):
        ...
        while True:
            ...
            if center_pixel.size > 0:
                ...
                # Display HSV values
                cv2.putText(frame, f"HSV: {avg_hsv}", (20, 40), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
                cv2.putText(frame, f"Detected: {color}", (20, 80), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                
                if debug_info:
                    cv2.putText(frame, f"Confidence: {debug_info['confidence']:.2f}", (20, 120), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            
            # Instructions
            cv2.putText(frame, "Hold cube color at center circle", (20, h-60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(frame, "Press 1-6 to save, S to save config, Q to quit", (20, h-20), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 2)
            
            # Show saved samples
            y_offset = 160
            for color_name, hsv_val in self.color_samples.items():
                cv2.putText(frame, f"{color_name}: {hsv_val}", (20, y_offset), 
    ...
    def generate_ranges_from_samples(self):
        ...
        for color_name, hsv in self.color_samples.items():
            ...
            if color_name == "white":
                ...
                if h <= 10:
                    new_ranges["red"] = ([max(0, h-10), max(50, s-30), max(50, v-30)], 
                                        [min(15, h+10), 255, 255])
                    new_ranges["red2"] = ([170, max(50, s-30), max(50, v-30)], [180, 255, 255])
                else:
                    new_ranges["red"] = ([max(170, h-10), max(50, s-30), max(50, v-30)], 
                                        [180, 255, 255])
    ...
    def test_detection(self):
        ...
        while True:
            ...
            if test_roi.size > 0:
                ...
                # Display results
                cv2.putText(frame, f"Detected: {color}", (20, 40), 
                           cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                
                if debug_info:
                    cv2.putText(frame, f"HSV: {debug_info['center_hsv']}", (20, 80), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                    cv2.putText(frame, f"Confidence: {debug_info['confidence']:.2f}", (20, 120), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
