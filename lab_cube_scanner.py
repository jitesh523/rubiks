...
class LABCubeScanner:
    ...
    def detect_face_lab(self, roi):
        ...
        if self.knn_model is None:
            return [["unknown" for _ in range(3)] for _ in range(3)], [[0.0 for _ in range(3)] for _ in range(3)]
    ...
    def display_info(self, frame, colors, confidences):
        ...
        # Main title
        cv2.putText(frame, f"Scanning: {face_name}", (10, 40),
                   cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0), 3)
        
        # Face counter
        cv2.putText(frame, f"Face {self.current_face + 1}/6", (10, 80),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        
        # Average confidence
        avg_conf = np.mean([conf for row in confidences for conf in row])
        conf_color = (0, 255, 0) if avg_conf > 0.8 else (0, 255, 255) if avg_conf > 0.6 else (0, 0, 255)
        cv2.putText(frame, f"Confidence: {avg_conf:.1%}", (10, 120),
        ...
        cv2.putText(frame, f"Quality: {quality}", (10, 160),
