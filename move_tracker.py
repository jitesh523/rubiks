...
class MoveTracker:
    ...
    def speak(self, text):
        ...
        cv2.putText(
            canvas,
            "Press SPACE when move is complete",
            (100, 320),
        ...
        cv2.putText(canvas, "Press 'q' to quit", (100, 380), cv2.FONT_HERSHEY_SIMPLEX, 0.7, blue, 2)
    ...
    def display_move_instruction(self, move, next_move, current_step, total_steps):
        ...
        cv2.putText(canvas, "Press 'q' to quit", (100, 410), cv2.FONT_HERSHEY_SIMPLEX, 0.7, blue, 2)
    ...
    def guide_through_solution(self, solver):
        ...
        while not solver.is_complete():
            ...
            instruction_canvas = self.display_move_instruction(
                current_move, next_move, current + 1, total
            )
    ...
    def show_completion_screen(self):
        ...
        cv2.putText(canvas, "CONGRATULATIONS!", (180, 100), cv2.FONT_HERSHEY_SIMPLEX, 1.5, gold, 3)
        cv2.putText(
            canvas,
        ...
        cv2.putText(
            canvas,
            "Well done! You followed all the moves correctly.",
            (120, 220),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.8,
            white,
        ...
        cv2.putText(
            canvas,
            "Your cube should now be in the solved state.",
            (130, 260),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.8,
            white,
        ...
        cv2.putText(
            canvas, "Press any key to exit", (280, 350), cv2.FONT_HERSHEY_SIMPLEX, 0.7, white, 2
        )
    ...
    def quick_move_guide(self, solver):
        ...
        for move in solver.solution_moves:
            ...
            user_input = input(
                f"    Press Enter when move {move_count} is complete (or 'q' to quit): "
            )
