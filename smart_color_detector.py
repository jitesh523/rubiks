...
class SmartColorDetector:
    ...
    def detect_single_color(self, roi: np.ndarray) -> tuple[str, float, dict]:
        ...
        for color_name in self.color_ranges.keys():
            ...
            if self.debug:
                print(
                    f"  {color_name:6s}: mask={mask_conf:.3f}, dist={dist_conf:.3f}, hybrid={hybrid_conf:.3f}"
                )
        ...
        for i in range(3):
            for j in range(3):
                if i < len(colors) and j < len(colors[i]):
                    ...
                    circle_color = (
                        (0, 255, 0)
                        if conf > self.good_confidence
                        else (0, 255, 255) if conf > self.min_confidence else (0, 0, 255)
                    )
...
def create_smart_detector(
    calibration_file="cube_calibration.json", debug=False
) -> SmartColorDetector:
